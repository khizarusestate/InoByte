                    ...
                    <article className="md:h-[90vh] md:w-[40vw] flex flex-col justify-center items-center gap-2.5 ">
                     {
                       Blogs.map((items,index)=>(
                            <div key={index} className="flex justify-center items-center gap-2.5">
                                <motion.img initial={{y:60}} animate={{y:0}} transition={{duration:0.5}} src={`./Images/${items.img}`} alt={items.Title} className="md:h-25 h-15 w-30 border border-purple-300 rounded-[5px] duration-100 hover:drop-shadow-[0_0_20px_rgb(255,255,255)] hover:scale-110 cursor-pointer"/>
